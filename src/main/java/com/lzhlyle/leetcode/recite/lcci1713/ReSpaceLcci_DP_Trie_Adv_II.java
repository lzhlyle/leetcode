...
public class ReSpaceLcci_DP_Trie_Adv_II {
    // reverse trie
    public int respace(String[] dictionary, String sentence) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = i - 1; j >= 0; j--) { // faster
                TrieNode node = trie.search(arr, j, i);
                if (node == null) break; // pruning
                if (node.isEnd()) dp[i] = Math.min(dp[i], dp[j]);
    ...
    class Trie {
        ...
        public TrieNode search(char[] arr, int l, int r) {
            TrieNode curr = root;
            for (int i = r - 1; i >= l; i--) {
                char c = arr[i];
                if (!curr.contains(c)) return null;
                curr = curr.get(c);
            }
            return curr;
        }
